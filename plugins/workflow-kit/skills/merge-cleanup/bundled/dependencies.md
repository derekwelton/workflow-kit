# Bundled dependencies

Read only the instructions needed by the current workflow. Preserve the user's scope,
repository contracts, independent review, and human acceptance boundaries.

- code-review: [instructions](./skills/code-review/INSTRUCTIONS.md)
- codebase-design: [instructions](./skills/codebase-design/INSTRUCTIONS.md)
- implement: [instructions](./skills/implement/INSTRUCTIONS.md)
- orchestrate (orchestrate-queue): [instructions](./skills/orchestrate/INSTRUCTIONS.md)
- ponytail: [instructions](./skills/ponytail/INSTRUCTIONS.md)
- research: [instructions](./skills/research/INSTRUCTIONS.md)
- tdd: [instructions](./skills/tdd/INSTRUCTIONS.md)
- update-issue: [instructions](./skills/update-issue/INSTRUCTIONS.md)
