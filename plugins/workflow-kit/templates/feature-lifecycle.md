...
<!-- workflow-kit:managed-start version=0.9.0 -->
...
Local repository overrides and frontmatter own tracker behavior. `tracker` may
be `github`, `github-projects`, or `linear`; an existing `linearTeam` without
`tracker` still enables Linear. No binding means ordinary GitHub Issues.
Conflicting `tracker` and `linearTeam` values block tracker writes.

For GitHub Projects, configure `githubProject.owner`, `number`, field names,
and `statuses.todo`, `inProgress`, `inReview` (optionally `codeReview`), plus
Issue Types and branch convention. Follow the installed `github-projects`
skill. Classify through Issue Type and Project Priority; labels are context.
Resolve field/option IDs live and use one issue card per unit of work. If there
is no AI-review column, keep the project In Progress during independent review
and record that phase in the manifest. Workload human review still waits for
combined verification. Never set Done or overwrite custom status mappings.
Routine refresh preserves tracker configuration and local overrides.

## Model and host policy

Use the installed `model-routing` skill and its versioned policy. Explicitly
select models/efforts rather than inheriting machine defaults. Normal coding:
Astra low, medium if needed; simple coding: Terra low or medium. Fable 5.1 may
delegate coding and visual implementation to Astra. Review normally uses medium.
High is reserved for orchestration or intense reasoning with a recorded reason.
Never use xhigh, max, or ultra. Preserve independent provider pairing and fresh
review context. Count coordinator and nested workers against host capacity;
queue excess work and respect the host's delegation authorization.
