...
export const CLAUDE_REVIEW_HIGH_REASON = "Owner-selected Opus 5 high for independent review (2026-09-13).";
export const MODELS = {
...
export function validateReviewAvailability(fallback, unavailableProvider) {
  ...
  if (fallback.reason === "review-models-unavailable" && fallback.unavailableProvider === unavailableProvider) {
    const expected = unavailableProvider === "claude" ? ["claude-opus-5", "claude-fable-5-1"] : ["gpt-6-astra"];
...
export function resolveRouting({ provider = "codex", task = "coding", model, effort, highReason, availableModels, reviewFallback = null } = {}) {
  ...
  let requestedModel = model ?? (provider === "codex" ? (task === "simple" ? "terra" : "astra") : task === "review" ? "opus" : "fable");
  ...
  const opusReview = task === "review" && selected.id === MODELS.opus.id;
  const selectedEffort = effort ?? (opusReview ? "high" : ["orchestration", "intense", "review"].includes(task) ? "medium" : "low");
  const reason = highReason ?? (opusReview && selectedEffort === "high" ? CLAUDE_REVIEW_HIGH_REASON : null);
