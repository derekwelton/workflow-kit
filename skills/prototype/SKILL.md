...
6. **Capture it when done.** Fold any validated decision into the real code, then capture the prototype itself as a **primary source**: commit it to a throwaway branch, out of main, and apply `update-issue` to the implementation issue with the question, verdict, recommendation/decision needed, and a pointer only after that branch is pushed and reachable. A local-only prototype path is not a usable pointer. The main branch keeps only the validated decision.
