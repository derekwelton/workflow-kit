...
test("Claude review defaults to owner-selected Opus high and falls back in declared order", () => {
  ...
  assert.equal(primary.model, "claude-opus-5");
  ...
  const unavailableClaude = { reason: "review-models-unavailable", unavailableProvider: "claude", attempts: [
    { model: "claude-opus-5", reason: "quota-unavailable", evidence: "Opus quota exhausted" },
  ...
  const claude = resolveRouting({ provider: "codex", task: "review", availableModels: ["claude-opus-5", "claude-fable-5-1"], reviewFallback: unavailableCodex });
  ...
  assert.equal(resolveRouting({ provider: "claude", task: "review", availableModels: ["claude-opus-5"], reviewFallback: unavailableClaude }).reviewFallback, null);
  ...
  assert.equal(resolveRouting({ task: "review", availableModels: ["gpt-6-astra", "claude-opus-5"] }).provider, "codex");
});
